"""
Category Management AI Analyzer
--------------------------------
App de Streamlit para analizar categorías de retail/trade marketing:
ABC/Pareto, market share, tendencias, surtido, insights narrativos con IA
(Claude API) y chat en lenguaje natural sobre los datos.
...
def validar_api_key(api_key: str):
    """Devuelve (key_limpia, error). Si error no es None, la key no es usable."""
    if not api_key:
        return None, None  # simplemente no hay key configurada, no es un error
    key = api_key.strip()
    if not key.isascii():
        return None, (
            "La API key contiene caracteres no válidos (por ejemplo un emoji o texto "
            "pegado de más). Borrá el campo y pegá SOLO la key, sin nada alrededor."
        )
    if not key.startswith("sk-ant-"):
        return None, (
            "Eso no tiene formato de API key de Anthropic (debería empezar con "
            "'sk-ant-'). Generá una nueva en platform.claude.com/settings/keys."
        )
    return key, None


def get_client():
    api_key_raw = st.session_state.get("api_key") or os.environ.get("ANTHROPIC_API_KEY")
    key, error = validar_api_key(api_key_raw)
    if error:
...
def extraer_texto(resp) -> str:
    """Junta solo los bloques de texto de la respuesta, ignorando bloques de
    'thinking' u otros tipos que puedan venir en modelos con razonamiento extendido."""
    partes = [b.text for b in resp.content if getattr(b, "type", None) == "text"]
    return "\n".join(partes).strip() if partes else "(El modelo no devolvió texto en la respuesta.)"
